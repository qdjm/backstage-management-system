...
const tablePropArr = [
  ...
  { prop: 'examineTime', label: '检查时间' },
  { prop: 'grade', label: '所得分数' },
  { prop: 'inspector', label: '检查人员' },
]
// 编辑表单显示信息
const formPropArr = [
  {
...
const getTableData = async () => {
  const res = await _axios.get(apiUrl.recordList)
...
watch(search, (newValue) => {
  tableData.dataList = tableData.backupsDataList.filter(
    (data: { [prop: string]: string }) => !search.value || data.canteenName.includes(newValue) || data.examineTime.includes(newValue) || data.grade.includes(newValue) || data.inspector.includes(newValue)
  )
})
// 改变分页的数量
...
const handleDelete = async (item: { id: number; }) => {
  await _axios.delete(`${apiUrl.recordList}/${item.id}`)
  await getTableData()
}
// 点击编辑按钮的回调函数
